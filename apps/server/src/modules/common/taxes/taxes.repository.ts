...
const tableName = "common_taxes";
const sortColumn = "name";
const columns = [
  { key: "code", type: "string" },
  { key: "name", type: "string" },
  { key: "tax_type", type: "string" },
...
@Injectable()
export class TaxesRepository implements OnModuleDestroy {
  ...
  public async get(id: number) {
    ...
    if (!row) throw new NotFoundException("Taxes record " + id + " was not found.");
  ...
  public async drop(id: number) {
    ...
    if (Number(result.numUpdatedRows) === 0) throw new NotFoundException("Taxes record " + id + " was not found.");
  ...
  public async forceDelete(id: number) {
    ...
    if (Number(result.numDeletedRows) === 0) throw new NotFoundException("Taxes record " + id + " was not found.");
