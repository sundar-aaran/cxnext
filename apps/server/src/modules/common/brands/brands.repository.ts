...
@Injectable()
export class BrandsRepository implements OnModuleDestroy {
  ...
  public async get(id: number) {
    ...
    if (!row) throw new NotFoundException("Brands record " + id + " was not found.");
  ...
  public async drop(id: number) {
    ...
    if (Number(result.numUpdatedRows) === 0) throw new NotFoundException("Brands record " + id + " was not found.");
  ...
  public async forceDelete(id: number) {
    ...
    if (Number(result.numDeletedRows) === 0) throw new NotFoundException("Brands record " + id + " was not found.");
