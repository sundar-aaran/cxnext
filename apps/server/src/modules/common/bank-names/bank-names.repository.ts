...
const tableName = "common_bank_names";
...
@Injectable()
export class BankNamesRepository implements OnModuleDestroy {
  ...
  public async get(id: number) {
    ...
    if (!row) throw new NotFoundException("BankNames record " + id + " was not found.");
  ...
  public async drop(id: number) {
    ...
    if (Number(result.numUpdatedRows) === 0) throw new NotFoundException("BankNames record " + id + " was not found.");
  ...
  public async forceDelete(id: number) {
    ...
    if (Number(result.numDeletedRows) === 0) throw new NotFoundException("BankNames record " + id + " was not found.");
