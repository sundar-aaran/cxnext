...
const tableName = "common_address_types";
...
@Injectable()
export class AddressTypesRepository implements OnModuleDestroy {
  ...
  public async get(id: number) {
    ...
    if (!row) throw new NotFoundException("AddressTypes record " + id + " was not found.");
  ...
  public async drop(id: number) {
    ...
    if (Number(result.numUpdatedRows) === 0) throw new NotFoundException("AddressTypes record " + id + " was not found.");
  ...
  public async forceDelete(id: number) {
    ...
    if (Number(result.numDeletedRows) === 0) throw new NotFoundException("AddressTypes record " + id + " was not found.");
